<?php
function vgwc_body_greek($post_id)
{
	...
	/* HTML Block */
	$vgwc_html .= '<div id="vgwc-wrapper'.$post_id.'" class="vgwc-wrapper theme-greek owl-carousel '.$vgwc_class_sufix.'">';
		if($vgwc_total_items)
		{
			for($i = 0; $i < $vgwc_total_loop; $i ++)
			{
				...
				for($m = 0; $m < $vgwc_row_carousel; $m ++)
				{
					...
					$vgwc_review	= $product->get_review_count() . esc_html__(" review(s)", "vgwc");
					...
					$vgwc_html .= '<div class="vgwc-item-i">';
						...
						if($vgwc_product_image)
						{
							$vgwc_html .= '<div class="vgwc-image-block">';
								...
								$vgwc_html .= '</a>';
								$vgwc_html .= ($vgwc_product_label && $vgwc_featured) ? '<div class="vgwc-label vgwc-featured">'.esc_html__("Hot", "vgwc").'</div>' : "";
								$vgwc_html .= ($vgwc_product_label && $vgwc_onsale) ? '<div class="vgwc-label vgwc-onsale">'.esc_html__("Sale", "vgwc").'</div>' : "";
								$vgwc_html .= '<div class="quick-wrapper">';
								$vgwc_html .= '<a class="quick-view quickview" data-quick-id="'.$vgwc_post_id.'" href="'.get_permalink().'" title="'.get_the_title().'">';
									$vgwc_html .= esc_html__("Quick view", "vgwc");
								$vgwc_html .= '</a>';
								$vgwc_html .= '</div>';
							$vgwc_html .= '</div>';
						}
	...
	if($vgwc_total_items)
	{
		...
		jQuery(document).ready(function($) {
			$("#vgwc-wrapper'.$post_id.'").owlCarousel({
				...
				navigationText: 	["'.esc_html__("Prev", "vgwc").'", "'.esc_html__("Next", "vgwc").'"],
