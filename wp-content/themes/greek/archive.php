					...
					<?php if (have_posts()) : ?>
						<header class="archive-header">
							<h1 class="archive-title"><?php
								if (is_day()) :
									printf(esc_html__('Daily Archives: %s', 'greek'), '<span>' . get_the_date() . '</span>');
								elseif (is_month()) :
									printf(esc_html__('Monthly Archives: %s', 'greek'), '<span>' . get_the_date(_x('F Y', 'monthly archives date format', 'greek')) . '</span>');
								elseif (is_year()) :
									printf(esc_html__('Yearly Archives: %s', 'greek'), '<span>' . get_the_date(_x('Y', 'yearly archives date format', 'greek')) . '</span>');
								else :
									esc_html_e('Archives', 'greek');
								endif;
							?></h1>
