...
add_action('wp_head','greek_woo_ajaxurl');
function greek_woo_ajaxurl() {
?>
...
add_action('wp_ajax_greek_product_quickview', 'greek_product_quickview');
add_action('wp_ajax_nopriv_greek_product_quickview', 'greek_product_quickview');

function greek_product_quickview() {
	global $product, $post, $woocommerce_loop, $greek_options;
	...
	<div class="woocommerce product">
		...
		<div class="product-info">
			...
			<a class="see-all" href="<?php echo esc_url($product->get_permalink()); ?>"><?php echo esc_html($greek_options['quickview_link_text']); ?></a>
			<div class="quick-add-to-cart">
				<?php woocommerce_template_single_add_to_cart(); ?>
			</div>
			<div class="quick-desc"><?php echo do_shortcode(get_post($productid)->post_excerpt); ?></div>
			<div class="social-sharing"><?php vinagecko_product_sharing(); ?></div>
		</div>
	</div>
	<?php
	die();
...
add_action('wp_ajax_greek_get_cartinfo', 'greek_get_cartinfo');
add_action('wp_ajax_nopriv_greek_get_cartinfo', 'greek_get_cartinfo');

function greek_get_cartinfo() {
	global $woocommerce;
...
add_action('wp_ajax_greek_get_productinfo', 'greek_get_productinfo');
add_action('wp_ajax_nopriv_greek_get_productinfo', 'greek_get_productinfo');

function greek_get_productinfo() {
	global $product, $woocommerce_loop, $greek_options;
	
	$productid = intval($_POST['data']['pid']);
	$product = get_product($productid);
	$quantity = intval($_POST['data']['quantity']);
	?>
	<h3><?php esc_html_e('Product is added to cart', 'greek');?></h3>
	...
	<div class="buttons">
		<a class="button" href="<?php echo get_permalink(wc_get_page_id('cart'));?>"><?php esc_html_e('View Cart', 'greek');?></a>
	</div>
	<?php
	die();
}