...
class Custom_WC_Widget_Cart extends WC_Widget_Cart {
	...
	public function __construct() {
		...
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'std'   => esc_html__('Cart', 'greek'),
				'label' => esc_html__('Title', 'greek')
			),
			'hide_if_empty' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide if cart is empty', 'greek')
			)
		);
		parent::__construct();
	}
	...
	public function widget($args, $instance) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? esc_html__('Cart', 'greek') : $instance['title'], $instance, $this->id_base);
