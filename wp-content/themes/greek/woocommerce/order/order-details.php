...
?>
<h2><?php esc_html_e('Order Details', 'greek'); ?></h2>
<table class="shop_table order_details">
	<thead>
		<tr>
			<th class="product-name"><?php esc_html_e('Product', 'greek'); ?></th>
			<th class="product-total"><?php esc_html_e('Total', 'greek'); ?></th>
		</tr>
	</thead>
	<tbody>
		<?php
							...
							<?php
								...
								if ($_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted()) {
									...
									foreach ($download_files as $download_id => $file) {
										...
										$links[] = '<small><a href="' . esc_url($file['download_url']) . '">' . sprintf(esc_html__('Download file%s', 'greek'), (count($download_files) > 1 ? ' ' . $i . ': ' : ': ')) . esc_html($file['name']) . '</a></small>';
									}
	...
	<?php
				...
				if ($has_refund && $key === 'order_total') {
					...
					if (wc_tax_enabled() && 'incl' == $order->tax_display_cart) {
						...
						if (! empty($tax_del_array)) {
							$refunded_tax_del .= ' ' . sprintf(esc_html__('(Includes %s)', 'greek'), implode(', ', $tax_del_array));
						}

						if (! empty($tax_ins_array)) {
							$refunded_tax_ins .= ' ' . sprintf(esc_html__('(Includes %s)', 'greek'), implode(', ', $tax_ins_array));
						}
					}
		...
		if ($has_refund) { ?>
			<tr>
				<th scope="row"><?php esc_html_e('Refunded:', 'greek'); ?></th>
		...
		if ('' != $order->customer_note) { ?>
			<tr>
				<th scope="row"><?php esc_html_e('Note:', 'greek'); ?></th>
...
<header>
	<h2><?php esc_html_e('Customer details', 'greek'); ?></h2>
</header>
<table class="shop_table shop_table_responsive customer_details">
<?php
	if ($order->billing_email) {
		echo '<tr><th>' . esc_html__('Email:', 'greek') . '</th><td data-title="' . esc_html__('Email', 'greek') . '">' . $order->billing_email . '</td></tr>';
	}

	if ($order->billing_phone) {
		echo '<tr><th>' . esc_html__('Telephone:', 'greek') . '</th><td data-title="' . esc_html__('Telephone', 'greek') . '">' . $order->billing_phone . '</td></tr>';
	}
...
<?php endif; ?>
		...
		<header class="title">
			<h3><?php esc_html_e('Billing Address', 'greek'); ?></h3>
		</header>
		<address>
			<?php
				if (! $order->get_formatted_billing_address()) {
					esc_html_e('N/A', 'greek');
				} else {
...
<?php if (! wc_ship_to_billing_address_only() && $order->needs_shipping_address() && get_option('woocommerce_calc_shipping') !== 'no') : ?>
	...
	<div class="col-2">
		...
		<header class="title">
			<h3><?php esc_html_e('Shipping Address', 'greek'); ?></h3>
		</header>
		<address>
			<?php
